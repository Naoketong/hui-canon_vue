	...
	<Layout>
		<div class="pg-main-header">
			...
			<el-radio-group style="margin-top:20px;" v-model="radio1" @change="choose">
				...
				<el-radio-button label="1">经济型</el-radio-button>
				<el-radio-button label="2">SUV</el-radio-button>
				<el-radio-button label="3">中级车</el-radio-button>
				<el-radio-button label="4">豪华型</el-radio-button>
				<el-radio-button label="5">MPV</el-radio-button>
				<el-radio-button label="6">6至15座商务车</el-radio-button>
			</el-radio-group>
				...
				<el-form class="handEdit" >
					...
					<el-form-item label="级别" label-width="60px">          
						<el-select v-model="formBoxValue.level">
							...
							<el-option label="中级车" :value="3" />
							<el-option label="豪华型" :value="4" />
							<el-option label="MPV" :value="5" />
							<el-option label="6至15座商务车" :value="6" />
						</el-select>
					</el-form-item>
					<el-form-item label="状态" label-width="60px">
						<el-select v-model="formBoxValue.state">
							<el-option label="空闲" :value="1" />
							<el-option label="租出" :value="2" />
		...
		<div class="pg-main-body">
			<el-table
				...
				<el-table-column
					...
					label="级别">
					<template slot-scope="scope" >
						{{ scope.row.level === 1 ? "经济型" : "" }}
						{{ scope.row.level === 2 ? "SUV" : "" }}
						{{ scope.row.level === 3 ? "中级车" : "" }}
						{{ scope.row.level === 4 ? "豪华型" : "" }}
						{{ scope.row.level === 5 ? "MPV" : "" }}
						{{ scope.row.level === 6 ? "6至15座商务车" : "" }}
					</template>
				</el-table-column>
					<el-table-column
					prop="state"
					label="状态">
					<template slot-scope="scope" >
						<el-tag v-if="scope.row.state === 1" type="success">空闲</el-tag>
						<el-tag v-if="scope.row.state === 2">租出</el-tag>
						<el-tag v-if="scope.row.state === 3" type="danger">维修保养</el-tag>
					</template>
				</el-table-column>
				<el-table-column
	...
	export default {
		...
		methods: {
			...
			handleSave() {
				...
				console.log(car_displacement,'排量')
				...
				if(!car_name || !car_displacement || !car_structure || !!car_img || !level || !state){
					this.$message.error('缺少必要车辆参数')
				  return
				}
				if(!price || !cost_basis || !cost_servic ||  !cost_insurance){
				  this.$message.error('缺少必要费用参数')
				  return
				}
				// 修改
				if(id){
